...
2. **IPv4→IPv6 Translation with Zero Checksum**  
   - Input: IPv4 UDP packet with checksum = 0x0000 (allowed in IPv4)
   - Expected: IPv6 UDP packet with non-zero checksum (required in IPv6)
   - Validates: `yanet_udp_checksum_v4_to_v6` function ensures mandatory IPv6 checksum
...
4. **IPv6→IPv4 Translation with 0xffff Checksum**
   - Input: IPv6 UDP packet with checksum = 0xffff
   - Expected: IPv4 UDP packet with checksum = 0x0000 (per RFC 6145)
   - Validates: `yanet_udp_checksum_v6_to_v4` function fix for 0xffff→0x0000 conversion
